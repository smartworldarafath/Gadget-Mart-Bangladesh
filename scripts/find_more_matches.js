...
function test() {
  ...
  // Search for later index occurrences of HT-S40R
  const model = "HT-S40R";
  let idx = 70000;
  let count = 0;
  while ((idx = html.indexOf(model, idx)) !== -1 && count < 15) {
    count++;
    console.log(`\nMatch #${count} (position ${idx}):`);
    console.log(`Context: ... ${html.substring(Math.max(0, idx - 100), Math.min(html.length, idx + 350))} ...`);
    idx += model.length;
  }
}
