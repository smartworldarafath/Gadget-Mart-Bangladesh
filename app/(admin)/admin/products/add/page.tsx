...
export default function AdminAddProductPage() {
  ...
  const [specs, setSpecs] = useState<{ key: string; value: string }[]>([
    { key: 'Connection Type', value: 'Wireless' },
    { key: 'Warranty Support', value: '1 Year Brand Warranty' }
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sku = form.sku || `GMB-${Date.now().toString().slice(-6)}-ADD`
    ...
    <div className="max-w-4xl mx-auto space-y-6 text-white select-none">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Configure product inventory parameters and specifications</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-gray-900 border border-gray-800 rounded-3xl p-6 md:p-8">
        ...
        <div className="pt-4 border-t border-gray-800">
          ...
          >
            {isLoading ? <RefreshCw className="animate-spin" size={16} /> : <Save size={16} />} Save Product
          </button>
        </div>
      </form>
    </div>
